...
// =============================================================================
// isSpeakerTag with allow-list integration tests
// =============================================================================

func TestIsSpeakerTag_AllowListOverridesHeuristic(t *testing.T) {
	// When allow-list is loaded, speakers in the list should return true
	// even if the heuristic would reject them (e.g., "Moongore" is fine
	// by heuristic, but this tests the allow-list path explicitly).
	al := &SpeakerAllowList{
	...
	SetSpeakerAllowList(al)
	defer SetSpeakerAllowList(nil) // restore

	speakers := []string{"Braxo", "Moongore", "Snell", "Yx"}
	for _, s := range speakers {
		if !isSpeakerTag(s) {
			t.Errorf("isSpeakerTag(%q) = false with allow-list, want true", s)
		}
	}
}

func TestIsSpeakerTag_AllowListRejectsNonSpeakers(t *testing.T) {
	// Tags NOT in the allow-list should still go through heuristic
	// Rejected animation tags should NOT become speakers via allow-list
	al := &SpeakerAllowList{
		lookup: map[string]bool{
			"braxo": true,
			"snell": true,
	...
	SetSpeakerAllowList(al)
	defer SetSpeakerAllowList(nil)

	// These are rejected tags (not in allow-list). The heuristic would
	// accept some of them (proper case) but allow-list doesn't have them.
	// However, isSpeakerTag should still apply heuristic for non-allow-list tags.
	// "Shocked" passes heuristic (proper case) but is animation tag.
	// With allow-list loaded, non-listed tags should still use heuristic.

	// Game commands should still be rejected
	commands := []string{"UpdateEntities", "PlaySFX", "FadeToBlack"}
	for _, cmd := range commands {
		if isSpeakerTag(cmd) {
			t.Errorf("isSpeakerTag(%q) = true with allow-list, want false (game command)", cmd)
		}
	}
}

func TestIsSpeakerTag_NilAllowListPreservesHeuristic(t *testing.T) {
	// When no allow-list is set, existing heuristic behavior is unchanged
	SetSpeakerAllowList(nil)

	// Proper case names still pass via heuristic
	if !isSpeakerTag("Braxo") {
		t.Error("isSpeakerTag('Braxo') should be true via heuristic")
	}
	// Ability scores still pass
	if !isSpeakerTag("wis") {
		t.Error("isSpeakerTag('wis') should be true via heuristic")
	}
	// Game commands still fail
	if isSpeakerTag("PlaySFX") {
		t.Error("isSpeakerTag('PlaySFX') should be false")
	}
}

func TestSetSpeakerAllowList(t *testing.T) {
	// Test that SetSpeakerAllowList sets and clears the global allow-list
	al := &SpeakerAllowList{
		lookup: map[string]bool{"test": true},
	}
	SetSpeakerAllowList(al)
	if globalAllowList == nil {
		t.Error("globalAllowList should be set after SetSpeakerAllowList")
	}
	SetSpeakerAllowList(nil)
	if globalAllowList != nil {
		t.Error("globalAllowList should be nil after SetSpeakerAllowList(nil)")
	}
}
