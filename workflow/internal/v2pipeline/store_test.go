...
func TestGetNextLines_ReturnsNextSourceRaw(t *testing.T) {
	store := testStore(t)
	items := []contracts.V2PipelineItem{
		{ID: "k/g-0/blk-0", SortIndex: 0, Knot: "k", Gate: "g-0", SourceRaw: "Line 1", SourceHash: "h1", State: StatePendingTranslate},
		{ID: "k/g-0/blk-1", SortIndex: 1, Knot: "k", Gate: "g-0", SourceRaw: "Line 2", SourceHash: "h2", State: StatePendingTranslate},
		{ID: "k/g-1/blk-0", SortIndex: 2, Knot: "k", Gate: "g-1", SourceRaw: "Next 1", SourceHash: "h3", State: StatePendingTranslate},
		{ID: "k/g-1/blk-1", SortIndex: 3, Knot: "k", Gate: "g-1", SourceRaw: "Next 2", SourceHash: "h4", State: StatePendingTranslate},
		{ID: "k/g-1/blk-2", SortIndex: 4, Knot: "k", Gate: "g-1", SourceRaw: "Next 3", SourceHash: "h5", State: StatePendingTranslate},
		{ID: "k/g-1/blk-3", SortIndex: 5, Knot: "k", Gate: "g-1", SourceRaw: "Next 4", SourceHash: "h6", State: StatePendingTranslate},
	}
	if _, _, err := store.Seed(items); err != nil {
		t.Fatalf("seed: %v", err)
	}

	lines, err := store.GetNextLines("k", "g-0", 3)
	if err != nil {
		t.Fatalf("GetNextLines: %v", err)
	}
	if len(lines) != 3 {
		t.Fatalf("expected 3 lines, got %d", len(lines))
	}
	if lines[0] != "Next 1" || lines[1] != "Next 2" || lines[2] != "Next 3" {
		t.Errorf("unexpected lines: %v", lines)
	}
}

func TestGetNextLines_EmptyInput(t *testing.T) {
	store := testStore(t)
	lines, err := store.GetNextLines("", "g-0", 3)
	if err != nil {
	...
	lines, err = store.GetNextLines("k", "", 3)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if lines != nil {
...
func TestGetNextLines_NoItemsAfter(t *testing.T) {
	store := testStore(t)
	items := []contracts.V2PipelineItem{
		{ID: "k/g-0/blk-0", SortIndex: 0, Knot: "k", Gate: "g-0", SourceRaw: "Last gate", SourceHash: "h1", State: StatePendingTranslate},
	}
	if _, _, err := store.Seed(items); err != nil {
		t.Fatalf("seed: %v", err)
	}
...
func TestGetAdjacentKO_ReturnsPrevAndNext(t *testing.T) {
	store := testStore(t)
	items := []contracts.V2PipelineItem{
		{ID: "k/blk-0", SortIndex: 0, Knot: "k", SourceRaw: "a", SourceHash: "ha", State: StateDone, KOFormatted: "가"},
		{ID: "k/blk-1", SortIndex: 1, Knot: "k", SourceRaw: "b", SourceHash: "hb", State: StateDone, KOFormatted: "나"},
		// batch range: sort_index 2-3
		{ID: "k/blk-2", SortIndex: 2, Knot: "k", SourceRaw: "c", SourceHash: "hc", State: StatePendingTranslate},
		{ID: "k/blk-3", SortIndex: 3, Knot: "k", SourceRaw: "d", SourceHash: "hd", State: StatePendingTranslate},
		{ID: "k/blk-4", SortIndex: 4, Knot: "k", SourceRaw: "e", SourceHash: "he", State: StateDone, KOFormatted: "마"},
		{ID: "k/blk-5", SortIndex: 5, Knot: "k", SourceRaw: "f", SourceHash: "hf", State: StateDone, KOFormatted: "바"},
	}
	if _, _, err := store.Seed(items); err != nil {
		t.Fatalf("seed: %v", err)
	}

	prevKO, nextKO, err := store.GetAdjacentKO("k", 2, 3, 3)
	if err != nil {
		t.Fatalf("GetAdjacentKO: %v", err)
	}
	if len(prevKO) != 2 {
		t.Fatalf("expected 2 prevKO, got %d: %v", len(prevKO), prevKO)
	}
	// prevKO is ordered DESC (closest first)
	if prevKO[0] != "나" || prevKO[1] != "가" {
		t.Errorf("unexpected prevKO: %v", prevKO)
	}
	if len(nextKO) != 2 {
		t.Fatalf("expected 2 nextKO, got %d: %v", len(nextKO), nextKO)
	}
	if nextKO[0] != "마" || nextKO[1] != "바" {
		t.Errorf("unexpected nextKO: %v", nextKO)
	}
}

func TestGetAdjacentKO_SkipsNullKOFormatted(t *testing.T) {
	store := testStore(t)
	items := []contracts.V2PipelineItem{
		{ID: "k/blk-0", SortIndex: 0, Knot: "k", SourceRaw: "a", SourceHash: "ha", State: StateDone, KOFormatted: "가"},
		{ID: "k/blk-1", SortIndex: 1, Knot: "k", SourceRaw: "b", SourceHash: "hb", State: StateDone}, // no ko_formatted
		{ID: "k/blk-2", SortIndex: 2, Knot: "k", SourceRaw: "c", SourceHash: "hc", State: StatePendingTranslate},
	}
	if _, _, err := store.Seed(items); err != nil {
		t.Fatalf("seed: %v", err)
	...
	prevKO, _, err := store.GetAdjacentKO("k", 2, 2, 3)
	if err != nil {
		t.Fatalf("GetAdjacentKO: %v", err)
	}
	// blk-1 has NULL ko_formatted, should be skipped
	if len(prevKO) != 1 {
		t.Fatalf("expected 1 prevKO (skipped NULL), got %d: %v", len(prevKO), prevKO)
	}
	if prevKO[0] != "가" {
		t.Errorf("unexpected prevKO: %v", prevKO)
	}
...
func TestGetAdjacentKO_NoAdjacentDone(t *testing.T) {
	store := testStore(t)
	items := []contracts.V2PipelineItem{
		{ID: "k/blk-0", SortIndex: 0, Knot: "k", SourceRaw: "a", SourceHash: "ha", State: StatePendingTranslate},
		{ID: "k/blk-1", SortIndex: 1, Knot: "k", SourceRaw: "b", SourceHash: "hb", State: StatePendingTranslate},
	}
	if _, _, err := store.Seed(items); err != nil {
		t.Fatalf("seed: %v", err)
	}

	prevKO, nextKO, err := store.GetAdjacentKO("k", 0, 1, 3)
	if err != nil {
		t.Fatalf("GetAdjacentKO: %v", err)
	}
	if len(prevKO) != 0 {
		t.Errorf("expected empty prevKO, got %v", prevKO)
	}
	if len(nextKO) != 0 {
		t.Errorf("expected empty nextKO, got %v", nextKO)
	}
}

// --- Seed ParentChoiceText test ---

func TestSeedParentChoiceText(t *testing.T) {
	store := testStore(t)
	items := []contracts.V2PipelineItem{
		{ID: "k/g-0/c-0/blk-0", SortIndex: 0, SourceRaw: "text", SourceHash: "hpct", State: StatePendingTranslate, ParentChoiceText: "Go north"},
	}
	...
	item, err := store.GetItem("k/g-0/c-0/blk-0")
	if err != nil {
		t.Fatalf("get item: %v", err)
	}
	if item.ParentChoiceText != "Go north" {
		t.Errorf("expected ParentChoiceText='Go north', got %q", item.ParentChoiceText)
	}
}

// --- ClaimBatch returns ParentChoiceText ---

func TestClaimBatchReturnsParentChoiceText(t *testing.T) {
	store := testStore(t)
	items := []contracts.V2PipelineItem{
		{ID: "k/g-0/c-0/blk-0", SortIndex: 0, Knot: "k", Gate: "g-0", BatchID: "batch-1", SourceRaw: "text", SourceHash: "hcb", State: StatePendingTranslate, ParentChoiceText: "The door"},
	}
	if _, _, err := store.Seed(items); err != nil {
		t.Fatalf("seed: %v", err)
	}

	batchID, claimed, err := store.ClaimBatch(StatePendingTranslate, StateWorkingTranslate, "worker-1", 300)
	if err != nil {
		t.Fatalf("claim batch: %v", err)
	}
	if batchID != "batch-1" {
		t.Errorf("expected batch-1, got %s", batchID)
	}
	if len(claimed) != 1 {
		t.Fatalf("expected 1 claimed, got %d", len(claimed))
	}
	if claimed[0].ParentChoiceText != "The door" {
		t.Errorf("expected ParentChoiceText='The door', got %q", claimed[0].ParentChoiceText)
	}
}
