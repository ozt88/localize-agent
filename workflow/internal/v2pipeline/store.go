...
// ResetAllForRetranslation snapshots all done+failed items into retranslation_snapshots,
...
func (s *Store) ResetAllForRetranslation(nextGen int) (int, error) {
	...
	if _, err := s.db.Exec(createSnapshotTable); err != nil {
	...
	// SQL 2: reset all done+failed to pending_translate
	resetSQL := s.rebind(`
		UPDATE pipeline_items_v2
		...
		WHERE state IN ('done', 'failed')`)
	result, err := s.db.Exec(resetSQL, contracts.StatePendingTranslate, nextGen, now)
	if err != nil {
