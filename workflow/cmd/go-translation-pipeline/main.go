...
func main() {
	...
	fs.BoolVar(&cfg.CleanupStaleClaims, "cleanup-stale-claims", cfg.CleanupStaleClaims, "requeue expired working claims by lease and exit")
	fs.BoolVar(&cfg.RouteKnownFailedNoRow, "route-known-failed-no-row", cfg.RouteKnownFailedNoRow, "route known translator-no-row failed families back to pending_translate and exit")
	fs.BoolVar(&cfg.RouteOverlayUI, "route-overlay-ui", cfg.RouteOverlayUI, "route overlay/UI rows to the dedicated overlay translate lane and exit")
	fs.BoolVar(&cfg.RepairBlockedTranslate, "repair-blocked-translate", cfg.RepairBlockedTranslate, "re-evaluate blocked_translate rows and release those whose predecessor is already satisfied")
	fs.BoolVar(&cfg.ResetScoring, "reset-scoring", cfg.ResetScoring, "reset done translated rows back to pending_score with retry_count/score state cleared")
	...
	fs.BoolVar(&cfg.RequeueTranslateNoRowAsRetranslate, "requeue-translate-no-row-as-retranslate", cfg.RequeueTranslateNoRowAsRetranslate, "move failed rows with 'translator produced no done row' to pending_retranslate with retry_count reset and exit")
	...
	fs.StringVar(&cfg.WorkerRole, "worker-role", cfg.WorkerRole, "worker role: all|translate|overlay-translate|score")
