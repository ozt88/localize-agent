...
Esoteric Ebb(내러티브 cRPG, Ink 스크립트 기반)의 한국어 번역 파이프라인 v2. v1에서 발견된 근본 문제 — 소스 생성 단위와 게임 렌더링 단위의 불일치 — 를 해결하기 위해 ink JSON 트리를 대사 블록 단위로 파싱하고, 씬 단위 클러스터 번역 + 포맷터 LLM으로 태그를 복원하는 2단계 아키텍처. 77,816건 전량 재번역.
