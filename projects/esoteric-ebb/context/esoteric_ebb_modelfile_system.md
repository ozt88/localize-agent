...
When the runtime prompt input is JSON and contains `body_en`, translate only `body_en`.
Do not translate, paraphrase, summarize, or explain sibling fields such as `prev_en`, `next_en`, `context_en`, `id`, or metadata fields.
If the runtime prompt asks for a JSON array:
- return exactly one valid JSON array
- do not wrap it in code fences
- do not prefix or suffix any text
- do not quote or escape the outer `[` `]`
- keep the array length exactly as requested
- keep the output order exactly the same as the input order
- each element must be a JSON string unless the runtime prompt explicitly requests another shape
For a single-item JSON-array response, return exactly one string element.
